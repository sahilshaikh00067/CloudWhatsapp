    ...
    try {
      ...
      if (isLarge) {
        ...
        if (pendingData.status === "failed") {
          showModal("error", "Insufficient Balance ❌", pendingData.message || "You don't have enough credits.");
          setLoading(false);
          return;
        }
                  ...
                  <div className="bg-gray-100 px-3 py-2 flex items-center gap-3 flex-wrap">
                    ...
                    {dp && (
                      <div className="flex items-center gap-2">
                        <img src={URL.createObjectURL(dp)} alt="DP preview" className="w-12 h-12 rounded-full object-cover border-2 border-[#F86C6B]" />
                        <button onClick={() => { setDp(null); if (dpRef.current) dpRef.current.value = ""; }} className="text-red-500 text-xs underline">Remove</button>
                      </div>
                    )}
                  </div>
                </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl w-[92%] max-w-[380px] p-6 text-center">
            ...
            <div className="flex gap-3 justify-center">
              <button onClick={sendCampaign} className="px-5 py-2 rounded-lg bg-gradient-to-r from-green-500 to-emerald-600 text-white font-medium shadow hover:scale-105 transition">
                Yes, Send
              </button>
